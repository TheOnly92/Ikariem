...
<div class="dynamic" id="reportInboxLeft">
	<h3 class="header">Inspect Garrison</h3>
	<div class="content">
		<div class="centerButton">
			<a href="?view=armyGarrisonEdit&id=254479&position=5" class="button">Fire units</a>
...
<div id="unitConstructionList" class="dynamic"><h3 class="header">Construction queue <a class="help" href="/ikariem/unitDescription" title="Help"><span class="textLabel">Help</span></a></h3>
	<div class="content">
		...
		{foreach from=$training_current.units item=unit}
		<div class="army_wrapper" title="{$unit.name}">
			<div class="army ship s{$unit.sid}">
		...
		<div class="results" style="">
			<div class="progressbar"><div class="bar" id="buildProgress" title="{$training_current.percentage}%" style="width:{$training_current.percentage}%"></div></div>
			<div class="time" id="buildCountDown">{$training_current.countdown}<span class="textLabel"> until completion</span></div>
		</div>
		{if count($training) > 0}
			{foreach from=$training item=row name=training}
...
<div id="mainview">
	...
	<form id="buildForm"  action="/shipyard/buildShips?id={$town_id}&position={$position}" method="POST">
		...
		<div class="content">
			...
			{foreach from=$units item=unit}
				<li class="unit {$unit.unit_id}">
					<div class="unitinfo">
						<h4>{$unit.name}</h4>
						<a title="To the description of {$unit.name}" href="/ikipedia/unitDescription?unitId={$unit.id}"><img src="http://static.ikariem.org/img/characters/fleet/120x100/{$unit.unit_id}_r_120x100.gif" /></a>
