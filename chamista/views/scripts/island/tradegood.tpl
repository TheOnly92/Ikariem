...
<div id="resUpgrade" class="dynamic{if isset($construction) > 0} upgrading{/if}">
	<h3 class="header">{$island.trade_mine} <a class="help" href="?view=informations&articleId=10013&mainId=10013" title="Help"><span class="textLabel">Need help?</span></a></h3>
	<div class="content">
		<img src="http://static.ikariem.org/img/resources/img_{$island.trade_css}.jpg" alt="" />
		...
		Event.onDOMReady(function() {
			...
			tmpCnt.subscribe("finished", function() {
				setTimeout(function() {
					location.href="/island/tradegood/id/{/literal}{$island.island_id}{literal}";
				},2000);
			})
		});
		{/literal}
		...
		{else}
		<div class="buildingLevel"><span class="textLabel">Level: </span>{$island.island_resource_lvl}</div>
		...
		<input name="type" type="hidden" value="tradegood" />
		<div id="donate">
			...
			<div class="centerButton"><input type="submit" style="width:180px;" class="button" value="Donate for upgrade" /></div>
		</div>
...
<div id="mainview">
	<div class="buildingDescription">
		<h1>{$island.trade_mine}</h1>
		<p>{$description}</p>
	</div>
	<form id="setWorkers" action="/island/setWorkers?id={$island.island_id}&cityId={$tradegood.townId}" method="post">
	<div id="setWorkersBox" class="contentBox">
		<h3 class="header"><span class="textLabel">Assign Workers</span></h3>
		<div class="content">
			<ul>
				<li class="citizens">
					<span class="textLabel">Citizens: </span>
					<span class="value" id="valueCitizens">{$tradegood.freeCitizens}</span>
				</li>
				<li class="workers">
					<span class="textLabel">Workers: </span>
					<span class="value" id="valueWorkers">{$tradegood.workers}</span>
				</li>
				<li class="gain" title="Worker production:{$tradeProduction}" alt="Worker production:{$tradeProduction}"><span class="textLabel">Capacity: </span>
					<div id="gainPoints">
						<div id="resiconcontainer"><img id="resicon" src="http://static.ikariem.org/img/resources/icon_{$island.trade_css}.gif" width="25" height="20" /></div>
					</div>
					<div class="gainPerHour"><span id="valueResource"{if $island.overload} class="overcharged"{/if}>+{$tradeProduction}</span>
					<span class="timeUnit">per Hour</span></div>
				</li>
				<li class="costs">
	...
	<div class="content">
		<table cellpadding="0" cellspacing="0">
			<thead>
				<tr>
				<th>Player
					<a href="?view=tradegood&type=tradegood&id=1114&sortBy=name&order=asc" class="unicode">&uArr;</a>
					<a href="?view=tradegood&type=tradegood&id=1114&sortBy=name&order=desc" class="unicode">&dArr;</a>
				</th>
				<th>Town</th>
				<th>Level</th>
				<th>Workers</th>
				<th>Donated
					<a href="?view=tradegood&type=tradegood&id=1114&sortBy=donation&order=asc" class="unicode">&uArr;</a>
					<a href="?view=tradegood&type=tradegood&id=1114&sortBy=donation&order=desc" class="unicode">&dArr;</a>
				</th>
				<th>Actions</th>
				</tr>
			</thead>
			<tbody>
				...
				<tr class="{$class}">
					...
					<td class="actions">{if $town.owner == $tradegood.owner}&nbsp;{else}<a href="/diplomacyAdvisor/sendIKMessage?receiverId={$town.owner}"><img src="http://static.ikariem.org/img/interface/icon_message_write.gif" alt="Write message" /></a>{/if}</td>
				</tr>
				{/foreach}
			</tbody>
		</table>		
...
Event.onDOMReady(function() {{/literal}
	...
	var startCitizens = {$tradegood.freeCitizens};
	var startResourceWorkers = {$tradegood.workers};
	var startIncomePerTimeUnit = {$tradegood.income};
	...
	var valRes = {$tradegood.corruption} * {$smarty.const.SPEED} * (Math.min({$maxWorkers}, slider.actualValue) + Math.max(0, 0.25 * (slider.actualValue-{$maxWorkers})));
