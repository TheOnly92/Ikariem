 ...
 * Ikariem
 * Palace controller file.
 * Handle all actions redirected to the palace controller.
...
class PalaceController extends Rewaz_Controller_Action_Buildings {
    public function preDispatch() {
        ...
        if ($Town->getGroundBuilding($position) != Chamista_Model_Formula::PALACE) {
    ...
    public function indexAction() {
        ...
        if ($Town->isBeingExpanded($position)) {
            ...
            $row = $Buildings_Queue->fetchRow($Buildings_Queue->select()
                ...
                ->where('queue_type = ?',Chamista_Model_Formula::PALACE)
            ...
            $this->view->construction = array(
                ...
                'currentdate' => $now,
                'building' => Chamista_Model_Format::building_action(Chamista_Model_Formula::PALACE),
        ...
        $this->view->upgrade_cost = Chamista_Model_Formula::getBuildCost(Chamista_Model_Formula::PALACE,$this->view->build_lvl + 1,$User);
        $this->view->upgrade_time = Chamista_Model_Format::formatTimeFromArray(Chamista_Model_Formula::getBuildTime(Chamista_Model_Formula::PALACE,$this->view->build_lvl + 1,true));
        $this->view->build_name = Chamista_Model_Format::building_plaintext(Chamista_Model_Formula::PALACE);
        $this->view->css = 'ik_palace_'.VERSION.'.css';
        $this->view->body_id = 'palace';
    }
}
