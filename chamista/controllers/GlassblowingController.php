 ...
 * Ikariem
 * Glassblowing controller file.
 * Handle all actions redirected to the glassblowing controller.
 ...
 */
class GlassblowingController extends Rewaz_Controller_Action_Buildings {
// This protected property contains the Town model of the currently selected town
	...
	public function preDispatch() {
		...
		if ($this->Town->getGroundBuilding($position) != Chamista_Model_Formula::GLASSBLOWING) {
	...
	public function indexAction() {
		...
		if ($this->Town->isBeingExpanded($position)) {
			...
			$row = $Buildings_Queue->fetchRow($Buildings_Queue->select()
				...
				->where('queue_type = ?',Chamista_Model_Formula::GLASSBLOWING)
				->where('queue_pos = ?',$position));
			$this->view->construction = array(
				'startdate' => $row->queue_created,
				'enddate' => $row->queue_endtime,
				'currentdate' => $now,
				'building' => Chamista_Model_Format::building_action(Chamista_Model_Formula::GLASSBLOWING),
		...
		$this->view->upgrade_cost = Chamista_Model_Formula::getBuildCost(Chamista_Model_Formula::GLASSBLOWING,$this->view->build_lvl + 1,$this->User);
		$this->view->upgrade_time = Chamista_Model_Format::formatTimeFromArray(Chamista_Model_Formula::getBuildTime(Chamista_Model_Formula::GLASSBLOWING,$this->view->build_lvl + 1,true));
		$this->view->build_name = Chamista_Model_Format::building_plaintext(Chamista_Model_Formula::GLASSBLOWING);
		$this->view->body_id = 'glassblowing';
		$this->view->css = 'ik_glassblowing_'.VERSION.'.css';
	}
}